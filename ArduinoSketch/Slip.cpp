...
void Slip::handleByte(uchar c)
{
    ...
    if(inbuffer_ptr>=SLIP_MAXINBUFFERSIZE)
      inbuffer_ptr=0;
...
{
    if(Serial.availableForWrite()<(1+2*((int)len)))
      return false;//the esc chars will change the size a bit but who knows exacly. in hardwareserial for uno sz==64 --> 31 is the max sz of tx pkt to be safe.
